...
LEJEASIDE.insertAdjacentHTML ("beforeend", `<br><br><h3>Billeje i alt ${parseFloat(URLDATA.get('lejeudgift')).
toLocaleString('da-DK', { style: 'currency', currency: 'DKK' })}</h3>inkl. moms`);
...
sessionStorage.setItem("lejeudgift", parseFloat(URLDATA.get('lejeudgift')).
toLocaleString('da-DK', { style: 'currency', currency: 'DKK' }));

let TOTAL = parseFloat(URLDATA.get('lejeudgift'));
let UDSTYRSUDGIFT = 0.00;

const TOTALSIDE = document.getElementById("totalindhold");
visTotal();
...
function visTotal() {
    TOTALSIDE.innerHTML = `<br><h3>total ${TOTAL.toLocaleString('da-DK', { style:'currency', currency: 'DKK' })}</h3>inkl. moms`;
}